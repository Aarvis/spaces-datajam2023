...
import { UPDATE_SCOREBOARD_VIEWTIME } from '../../graphqlResolverTemplate.js';
import { globalApolloClient } from '../../App.js';
...
const EventTimeline = () => {
  ...
  let visibleTimeRef = useRef(visibleTime);
  let matchIdRef = useRef(null);
  ...
  async function updateScoreBoardViewTime(matchId, viewTime){
    try{
    let response = await globalApolloClient.mutate({mutation:UPDATE_SCOREBOARD_VIEWTIME, variables:{
      matchId:matchId,
      viewTime:viewTime, 
    }})
  ...
  useEffect(() => {
    ...
    timer = setInterval(() => {
      setVisibleTime(prevTime => prevTime + 1);
    }, 1000);


    return () => {
      clearInterval(timer);
      console.log('unmounter return')
      console.log('unmounted')
      console.log(visibleTimeRef.current)
      console.log('matchIdRef',matchIdRef)
      updateScoreBoardViewTime(matchIdRef.current ,visibleTimeRef.current);
    };
  }, []);
